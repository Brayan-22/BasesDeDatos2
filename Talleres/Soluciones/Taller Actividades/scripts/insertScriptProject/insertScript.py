...
personas = [(random.choice(nombres), random.choice(apellidos)) for i in range(1,500000)]
...
try:
    ...
    insert into tipoinconveniente(id_tipoinconveniente,nombre_tipoinconveniente) values 
    (%s,%s)
    """
    ...
    id_min = """
    select min(id_persona) from persona;"""
    id_max = """
    ...
    insert into actividad(id_persona,nombre_actividad,desc_actividad,horas_plan_actividad) values
    (%s,%s,%s,%s)
    """
    ...
    personas_aleatorias = [random.randint(min_persona,max_persona) for i in range(1,460000)]
    actividades_insert = [
        (
            personas_aleatorias[i-1],
            actividad,
            actividades_descripciones[actividad],
            random.randint(6, 24)
        )
    ...
    insert_avance= """
    insert into avance(id_avance,id_actividad,desc_avance,horas_avance)values
    (%s,%s,%s,%s)
    """
    id_min_act = """
    ...
    progreso_insert1 = [(1,ind,lorem.paragraph()[:20],random.randint(1,2)) for ind in range(min_actividad, max_actividad)]
    progreso_insert2 = [(2,ind,lorem.paragraph()[:20],random.randint(1,2)) for ind in range(min_actividad, max_actividad)]
    progreso_insert3 = [(3,ind,lorem.paragraph()[:20],random.randint(1,2)) for ind in range(min_actividad, max_actividad)]
    cursor.executemany(insert_avance, progreso_insert1)
    coneccion.commit()
    cursor.executemany(insert_avance, progreso_insert2)
    coneccion.commit()
    cursor.executemany(insert_avance, progreso_insert3)
    coneccion.commit()
    ##Fin insert avances
    ##insertar inconvenientes
    insert_inconvenientes = """
    insert into inconveniente(id_actividad,id_avance,id_inconveniente,id_tipoinconveniente,desc_inconveniente)values
    (%s,%s,%s,%s,%s)
    """
    inconvenientes_insert1 = [(i,1,1,random.randint(1,3),lorem.paragraph()[:15])for i in range(min_actividad, max_actividad)]
    inconvenientes_insert2 = [(i,2,2,random.randint(1,3),lorem.paragraph()[:15])for i in range(min_actividad, max_actividad)]
    cursor.executemany(insert_inconvenientes, inconvenientes_insert1)
    cursor.executemany(insert_inconvenientes, inconvenientes_insert2)
    coneccion.commit()
